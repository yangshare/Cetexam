...
public class TeacherAction {
	...
	public String tleft() {
		...
		teachers = (List<Teacher>) objectService
				.check("教师", username, password);
		return "tleft";
	}
	...
	static void upload() {
		try {
			...
			OutputStream os = new FileOutputStream(ServletActionContext
					.getRequest().getRealPath("images/teachers")
					+ "/" + imageFileName); // 输出数据
			int i = 0;
	...
	//添加教师的跳转
	public String link() {
		return "success";//跳到教师添加页面
	}
	//修改教师跳转action
	@SuppressWarnings("unchecked")
	public String linkUpdateTeacherPage() {
		curPage = 1;//修改成功跳到教师列表第一页
		this.teachers = objectService.page("Teacher", (curPage - 1)
	...
	public String update() {
		...
		Teacher teacher = (Teacher) objectService.getObjectById(Teacher.class,
				id);
